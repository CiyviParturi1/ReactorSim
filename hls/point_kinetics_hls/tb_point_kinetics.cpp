...
static bool finite_state(const ReactorSim& sim) {
    if (!isfinite(sim.t) || !isfinite(sim.n) || !isfinite(sim.Tf) || !isfinite(sim.Tc) || !isfinite(sim.I_Xe) || !isfinite(sim.Xe)) {
        return false;
    }
...
static int test_hls_top() {
    ...
    failures += expect(fabs(n_out - 1.0f) < 1.0e-5f, "HLS top starts near critical power");
    failures += expect(fabs(rod_position_out - rod_target_out) < 1.0e-6f, "HLS top initializes rod position at target");
    failures += expect(fabs(target_h_out - h) < 1.0e-8f, "HLS top reports target timestep");
    failures += expect(fabs(decay_heat_out - 0.066f) < 1.0e-5f, "HLS top initializes equilibrium grouped decay heat");
    ...
    failures += expect( t_out > 2400.0f, "HLS top advances beyond the former 2048-second float freeze");
#else
    failures += expect(t_out > 0.009f && t_out < 0.011f, "HLS top frame-step time advances in cosim-sized test");
#endif

    cout << "HLS top step" << endl;
    cout << "t=" << t_out << " N_out=" << n_out << " Tf=" << tf_out << " Tc=" << tc_out << " I_Xe=" << iodine_out << " Xe=" << xenon_out << " rod=" << rod_position_out
         << " C0_out=" << c_out[0] << endl;
...
int main() {
    ...
    cout << "t=" << sim.t << " n=" << sim.n << " Tf=" << sim.Tf << " Tc=" << sim.Tc << " I_Xe=" << sim.I_Xe << " Xe=" << sim.Xe << " rho=" << sim.get_total_rho() << " rod=" << sim.rod_position
         << endl;

    failures += expect(finite_state(sim), "initial state is finite");
    failures += expect(fabs(sim.get_total_rho()) < 1.0e-6f, "initial reactivity is critical");
    failures += expect(fabs((1.0f - sim.p.total_decay_fraction()) * sim.n + sim.decay_heat - 1.0f) < 1.0e-6f, "prompt plus decay heat is normalized at equilibrium");
    failures += expect(sim.tc.factor == 1.0f, "default time factor is 1x");
    failures += expect(fabs(sim.tc.h() - TimeCompression::BASE_H) < 1.0e-6f, "default timestep is BASE_H");
    ...
    cout << "t=" << sim.t << " n=" << sim.n << " rho=" << sim.get_total_rho() << " rod=" << sim.rod_position << " target=" << sim.rod_target << endl;
    ...
    failures += expect(fabs(sim.tc.h() - 0.001f) < 1.0e-8f, "training mode timestep is 0.001");
    ...
    failures += expect(fabs(sim.tc.h() - 0.002f) < 1.0e-8f, "xenon mode timestep is 0.002");
    ...
    failures += expect(fabs(sim.tc.h() - 0.002f) < 1.0e-8f, "custom factor clamps timestep");

    float decay_at_scram = sim.decay_heat;
    sim.scram();
    failures += expect(sim.rod_position == 0.0f && sim.rod_target == 0.0f, "SCRAM fully inserts rods");
    ...
    cout << "t=" << sim.t << " n=" << sim.n << " Tf=" << sim.Tf << " Tc=" << sim.Tc << " rho=" << sim.get_total_rho() << " rod=" << sim.rod_position << endl;
    ...
    failures += expect(sim.decay_heat < decay_at_scram, "grouped decay heat decreases after SCRAM");
    ...
    failures += expect(fabs(sim.get_total_rho()) < 1.0e-6f, "reset returns to critical rod configuration");

    sim.plant_mode = 1;
    sim.initialize(1.0f);
    failures += expect(sim.plant_mode == 1, "plant mode can switch to RBMK-like preset");
    failures += expect(fabs(sim.get_total_rho()) < 1.0e-6f, "plant preset reset returns to critical state");
