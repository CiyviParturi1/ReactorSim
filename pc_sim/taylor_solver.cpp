...
void run_simulation_host(ReactorSim& sim, float wall_output_interval) {
    ...
    const Clock::duration frame_period =
    ...
    while (true) {
            ...
            for (std::size_t index = 0; index < pending_commands.size(); ++index) {
                ...
                if (code == '+') {
                    ...
                    if (stream >> factor &&
                        ...
                        std::cerr << "[TIME] Custom requested=" << sim.tc.factor
                                  << "x  h=" << sim.tc.h() << "s\n";
                    }
                }
            }
        ...
        const double requested_advance =
            static_cast<double>(wall_output_interval) * sim.tc.factor;
        const double requested_step_count = requested_advance / h;
        int steps = pk::MAX_SUBSTEPS;
        if (std::isfinite(requested_step_count) &&
            requested_step_count < pk::MAX_SUBSTEPS) {
        ...
        const std::chrono::duration<double> compute_elapsed =
        ...
        const float rho_fuel_dlr =
            (config.alpha_f * (sim.Tf - sim.Tf_ref)) / beta;
        ...
        if (now < next_deadline) {
            ...
            std::cerr << "[TIME] Frame deadline missed; achieved physics factor="
                      << achieved_factor << "x\n";
        }
    }
}
...
int main() {
    ...
    std::cerr << "=== Point-Kinetics Reactor Simulator + Time Compression ===\n"
              ...
              << "TIME: M0=REALTIME(1x), M1=TRAINING(10x), "
                 "M2=XENON(1000x), T <factor>=CUSTOM\n"
